...
async function handlePrintPage() {
  // Re-render the current page without search highlighting
  const previewImage = await previewManager.renderPage('');
  ...
  if (isMobile) {
    try {
      ...
      // Create a new jsPDF instance with better defaults for mobile
      const pdf = new jsPDF({
      ...
      await new Promise((resolve, reject) => {
        img.onload = () => {
          try {
            ...
            // Calculate scaling to fit image properly on page
      ...
      Swal.fire({
        title: 'Error',
        text: 'Could not create PDF. Downloading as image instead.',
        icon: 'warning',
    ...
    // Existing desktop printing logic remains unchanged
  ...
  // Re-render the preview with highlights if there was a search term
  const searchInput = document.getElementById('searchInput');
...
function populateTermDropdown(terms) {
  ...
  termSelect.innerHTML = '<option value="" disabled selected>Select Students Name </option>';
...
function populateExamDropdown(exams) {
  // ... existing code ...
   const examSelect = document.getElementById('examSelect');
  examSelect.innerHTML = '<option value="" disabled selected>Select Document</option>';
  exams.forEach(exam => {
    const option = document.createElement('option');
    option.value = exam;
