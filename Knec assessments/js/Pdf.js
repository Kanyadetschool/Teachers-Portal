...
import { Uploads2024} from '../Uploads/Uploads2024.js';
...
class PdfPreviewManager {
  ...
  constructor() {
    ...
    this.currentScale = window.innerWidth <= 850 ? 1.5 : 5;
    this.rotation = 0;
    this.thumbnails = new Map();
    this.textContent = new Map();
    this.initialPinchDistance = 0;
    ...
    window.addEventListener('resize', () => {
        const newScale = window.innerWidth <= 850 ? 1.5 : 5;
        if (this.currentScale !== newScale) {
            this.currentScale = newScale;
            // Rerender pages if PDF is loaded
            if (this.currentPdf) {
                this.renderAllPages();
            }
        }
    ...
    async renderAllPages() {
        ...
        previewDiv.innerHTML = `
            ...
            <div class="toolbar">
                ...
                <div class="toolbar-group">
                <button id="printBtn" class="btn-control">🖨️ Print</button>
        ...
        document.getElementById("printBtn").addEventListener("click", async () => {
            try {
                const pdfData = await this.currentPdf.getData();
                ...
                const cleanup = () => {
                    if (!isCleanedUp) {
                        isCleanedUp = true;
                        URL.revokeObjectURL(blobUrl);
                        // Only close the window if printing was initiated and completed
                        if (isPrinting) {
                            printWindow.close();
                        }
                    }
                ...
                printWindow.onload = () => {
                    ...
                    printWindow.document.body.insertAdjacentHTML('afterbegin', `
                        <div style="position: fixed; top: 45px; right: 20px; z-index: 9999; background: white; padding: 10px; border: 1px solid #ccc; ">
                            <button onclick="window.print()" style="padding: 5px 10px;">Print</button>
                            <button onclick="window.close()" style="padding: 5px 10px; margin-left: 5px;">Close</button>
                        </div>
                    `);

                    // Setup print-related event listeners
                ...
                try {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Print Error',
                        text: 'Failed to print. Please try downloading and printing directly.',
                        timer: 3000
                    });
                }
            }
        ...
        pageNumberInput.addEventListener('keydown', async (event) => {
            if (event.key === 'Enter') {
                ...
                if (pageNumber >= 1 && pageNumber <= this.totalPages) {
                    ...
                    const previewImage = await this.renderPage();
...
async function updatePreviewDisplay(previewImage) {
    ...
    // Preserve existing pagination input value if it exists
    const existingInput = document.getElementById('pageNumberInput');
    const currentInputValue = existingInput ? existingInput.value : '';
            ...
            <div class="toolbar-group">
                ...
                <div class="pagination-container">
                    <input type="number" 
                        id="pageNumberInput" 
                        class="page-input"
                        value="${currentInputValue || currentPage}"
                        min="1" 
                        max="${totalPages}" 
                        placeholder="Page #"
                    />
    ...
    document.getElementById("printBtn").addEventListener("click", async () => {
        try {
            const pdfData = await previewManager.currentPdf.getData();
            const blob = new Blob([pdfData], { type: 'application/pdf' });
            const blobUrl = URL.createObjectURL(blob);
            const printWindow = window.open(blobUrl, '_blank');
            if (!printWindow) {
    ...
    // Re-attach the page number input event listener
    const pageNumberInput = document.getElementById('pageNumberInput');
    pageNumberInput.addEventListener('keydown', async (event) => {
        if (event.key === 'Enter') {
            const pageNumber = parseInt(pageNumberInput.value, 10);
            if (pageNumber >= 1 && pageNumber <= totalPages) {
                previewManager.currentPage = pageNumber;
                const newPreviewImage = await previewManager.renderPage();
                updatePreviewDisplay(newPreviewImage);
            } else {
                alert('Invalid page number');
                pageNumberInput.value = currentPage; // Reset to current page if invalid
            }
        }
    });
}
