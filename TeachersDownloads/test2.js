    ...
    image: async (url) => {
        ...
        container.innerHTML = `
            <div class="flex items-center justify-center h-full">
                <img src="${url}" class="max-w-full max-h-full object-contain" alt="Preview" />
            </div>`;
    },
    text: async (url) => {
        try {
            ...
            const container = document.getElementById('preview-container');
            container.innerHTML = `
                <pre class="whitespace-pre-wrap p-4 bg-gray-50 rounded h-full overflow-auto">
                    ${text}
                </pre>`;
        } catch (error) {
            showAlert("Failed to load text preview", "error");
        }
    }
};
