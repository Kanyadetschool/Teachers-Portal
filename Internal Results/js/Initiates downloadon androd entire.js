// Initiates downloadon androd entire pdf and desktop nonfall back after print user has to reselect class agin
...
async function handlePrintPage() {
  try {
    // First check if exam is selected
    ...
    if (!yearSelect || !yearSelect.value) {
      Swal.fire('Please Select', 'Please select a Year first', 'warning');
      return;
    }

    if (!termSelect || !termSelect.value) {
      Swal.fire('Please Select', 'Please select a Term first', 'warning');
      return;
    }

    if (!examSelect || !examSelect.value) {
      Swal.fire('Please Select', 'Please select a Student first', 'warning');
      return;
    }

    // Get the PDF URL
    const selectedExam = examSelect.value;
    if (!currentDataArray || !currentDataArray.files) {
    ...
    if (!pdfUrl) {
      Swal.fire('Error', 'No PDF file found for this selection', 'error');
      return;
    }
    ...
    if (isMobile) {
      try {
        // For mobile devices, open PDF directly in new window
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Unable to open PDF. Please check if popups are allowed.',
          confirmButtonText: 'Try Direct Download',
          ...
          if (result.isConfirmed) {
            ...
            link.href = pdfUrl;
            link.download = `${selectedExam}.pdf`;
    ...
    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: 'Unable to prepare document for printing. Please try again.',
      footer: 'If the problem persists, try downloading the file directly.'
    });
  } finally {
    // Ensure loading indicator is closed in case of errors
    setTimeout(() => {
      Swal.close();
    }, 1000);
  }
